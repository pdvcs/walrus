...
| Variable            | Zod | Note                                                                                                                                                                                                                                            |
| ------------------- | --- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `DEFAULT_RETENTION` | ✓   | Parsed and range-checked by the config schema, then read nowhere. Retention comes from each package's TOML, where `retention.versions_per_group` has its own default of 3 (`src/types/package-config.ts`). Setting this variable has no effect. |
